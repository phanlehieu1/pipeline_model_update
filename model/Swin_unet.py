...
def interpolate_relative_position_bias_table(table, src_window_size, tgt_window_size, num_heads):
    ...
    table = table.view(src_size, src_size, num_heads).permute(
        2, 0, 1).unsqueeze(0)  # 1, H, src, src
    table = F.interpolate(table, size=(tgt_size, tgt_size),
                          mode='bicubic', align_corners=False)
    table = table.squeeze(0).permute(1, 2, 0).reshape(
        tgt_size * tgt_size, num_heads)
    return table
...
def load_pretrained_encoder(model, checkpoint_path="swinv2_tiny_patch4_window8_256.pth"):
    ...
    # hoặc "state_dict" tùy vào file .pth
    ...
    print(
        f"Đã load {len(pretrained_dict)} trọng số từ ImageNet vào Swin-Unet.")
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               pretrained_window_size=pretrained_window_sizes[
                                   i_layer] if pretrained_window_sizes is not None else None,
