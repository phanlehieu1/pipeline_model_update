...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train, Pretrain hoặc Evaluate một model AI")
    ...
    parser.add_argument("--mode", type=str, choices=["train", "pretrain", "evaluate"],
                        required=True, help="Chế độ: train hoặc pretrain hoặc evaluate")
    parser.add_argument("--data", type=str, required=True,
                        help="Đường dẫn đến dataset đã giải nén")
    # Tham số trường hợp
    parser.add_argument("--checkpoint", type=str,
                        help="Đường dẫn đến file checkpoint (chỉ dùng cho chế độ pretrain và evaluate)")
    parser.add_argument("--augment", action='store_true',
                        help="Bật Augmentation cho dữ liệu đầu vào")
    ...
    parser.add_argument("--img_size", type=int, nargs=2,
                        help="Height and width of the image, default = [256, 256]")
    parser.add_argument("--numclass", type=int,
                        help="shape of class, default = 1")
    ...
    """
    parser.add_argument("--loss", type=str, choices=["Dice_loss", "BCEDice_loss"],
                        default="Dice_loss", help="Hàm loss sử dụng, default = Dice_loss")
    parser.add_argument("--optimizer", type=str, choices=[
                        "Adam", "SGD"], default="Adam", help="Optimizer sử dụng, default = Adam")
...
def main():
    ...
    Swin_unet.load_pretrained_encoder(
        model1, "swinv2_large_patch4_window12to16_192to256_22kto1k_ft.pth")
    ...
    if args.mode == "train":
        ...
        trainer.pretrained(train_loader=trainLoader, val_loader=validLoader,
                           test_loader=testLoader, checkpoint_path=args.checkpoint)
        # trainer.pretrained(trainLoader,validLoader,args.checkpoint)
        export(trainer)
    else:
        if not args.checkpoint:
            raise ValueError("Chế độ evaluate yêu cầu checkpoint!")
        trainer.evaluate(train_loader=trainLoader, val_loader=validLoader,
                         test_loader=testLoader, checkpoint_path=args.checkpoint)
        # trainer.pretrained(trainLoader,validLoader,args.checkpoint)
